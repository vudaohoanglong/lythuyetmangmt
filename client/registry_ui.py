...
class Registry_UI(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.breakline_1=tk.Label(self,text="-"*30+" Thay đổi trực tiếp từ file "+"-"*30,height=1,justify=tk.CENTER)
        self.breakline_1.grid(row=0,column=0,sticky=tk.N,padx=10,pady=10,columnspan=4)
        self.txt_regpath=tk.Text(self,width=64,height=2,bg='#FFFFFF')
        self.txt_regpath.grid(row=1,column=0,sticky=tk.N,padx=10,pady=10,columnspan=3)
        self.btn_browse=tk.Button(self,text='Browse',width=30,height=2)
        self.btn_browse.grid(row=1,column=3,sticky=tk.N,pady=10,padx=10)
        self.btn_browse["command"]=self.browser_path
        self.reg_box_text=tk.Text(self,width=64,height=10,bg="#FFFFFF")
        self.reg_box_text.grid(row=2,column=0,columnspan=3,sticky=tk.N,padx=10,pady=10)
        self.btn_send_cnt=tk.Button(self,text='Gửi nội dung',wraplength=50,width=30,height=10)
        self.btn_send_cnt.grid(row=2,column=3,sticky=tk.N,padx=10,pady=10)
        self.btn_send_cnt["command"]=self.send_reg_file
        #
        self.breakline_2=tk.Label(self,text="-"*30+" Thay đổi trực tiếp từ các chức năng "+"-"*30,height=1,justify=tk.CENTER)
        self.breakline_2.grid(row=3,column=0,sticky=tk.N,padx=10,pady=10,columnspan=4)
        #Thay doi tu cac chuc nang
        
        #path
        self.lb_path=tk.Label(self,text="Đường dẫn",width=20,justify=tk.CENTER)
        self.lb_path.grid(row=5,column=0,sticky=tk.E,padx=10,pady=10,columnspan=1)
        self.txt_path=tk.Text(self,width=50,height=1,bg="#FFFFFF")
        self.txt_path.grid(row=5,column=1,columnspan=3,padx=10,pady=10)
        #Name value
        self.lb_name=tk.Label(self,text="Name value",height=1,justify=tk.CENTER)
        self.lb_name.grid(row=6,column=0,padx=10,pady=10,columnspan=1)
        self.txt_name=tk.Text(self,width=25,height=1,bg="#FFFFFF")
        self.txt_name.grid(row=7,column=0,columnspan=1,pady=10,padx=10)
        #Value settings
        self.lb_value=tk.Label(self,text="Value",height=1,justify=tk.CENTER)
        self.lb_value.grid(row=6,column=1,padx=10,pady=10,columnspan=2)
        self.txt_value=tk.Text(self,width=25,height=1,bg="#FFFFFF")
        self.txt_value.grid(row=7,column=1,pady=10,padx=10,columnspan=2)
        #Select type
        self.lb_type=tk.Label(self,text="Data type",height=1,justify=tk.CENTER)
        self.lb_type.grid(row=6,column=3,padx=10,pady=10,columnspan=2)
        ...
        self.option_func.grid(row=4,column=1,padx=10,pady=10,columnspan=3)
